...
MOVIE_IDS = [10426, 12153, 584, 5174, 16555]
...
@bp.route("/fetch_comment", methods=["POST"])
def fetch_comment():

    user_comments = Comment.query.filter_by(username=current_user.username).all()
    num_comments = len(user_comments)
    return flask.jsonify(
        revised=[
            (
                user_comments[i].movieid,
                user_comments[i].rating,
                user_comments[i].comments,
                user_comments[i].username,
            )
...
# This route should update the existing review and if otherwise delete the review if needed.
    ...
    for i in range(num_comments):
        for j in comment_data:
            if i == j:
                ...
                db.session.commit()
    return ("Your changes have been successfully added.", print(comment_data))
...
def register():
    ...
    if form.validate_on_submit():
        ...
        new_user = User(
            username=form.username.data, password=hashed_password.decode("utf-8")
        )
